...
#include "gatt_access.h"    /* GATT-related routines *///"ibeacon_service.h"
...
#include "eddystone\eddystone_service.h" /* Beacon service interface */
#include "eddystone\eddystone_adv.h"      /* Beacon routines */
#include "ibeacon\ibeacon_service.h"
#include "ibeacon\ibeacon_adv.h"
/*============================================================================*
 *  Private Definitions
...
#define NVM_OFFSET_SANITY_WORD         (0)                          //0xf000

/* NVM offset for bonded flag */
#define NVM_OFFSET_BONDED_FLAG         (NVM_OFFSET_SANITY_WORD + 1) //0xf002

/* NVM offset for bonded device Bluetooth address */
#define NVM_OFFSET_BONDED_ADDR         (NVM_OFFSET_BONDED_FLAG + \
                                        sizeof(g_app_data.bonded))  //0xf004(5B)

/* NVM offset for diversifier */
#define NVM_OFFSET_SM_DIV              (NVM_OFFSET_BONDED_ADDR + \
                                        sizeof(g_app_data.bonded_bd_addr))//0xf00e(1B)

/* NVM offset for IRK */
#define NVM_OFFSET_SM_IRK              (NVM_OFFSET_SM_DIV + \
                                        sizeof(g_app_data.diversifier))//0xf010(8B)

#define NVM_OFFSET_BEACON_CONFIG_FLAG   (NVM_OFFSET_SM_IRK + MAX_WORDS_IRK)//0xf020(1B)
/* Number of words of NVM used by application. Memory used by supported 
 * services is not taken into consideration here.
 */     
//first user customisation service data start from here:GAP service
#define NVM_MAX_APP_MEMORY_WORDS       (NVM_OFFSET_BEACON_CONFIG_FLAG + 1)//0XF022
/*#define NVM_OFFSET_BAT_SERVICE_DATA    (NVM_MAX_APP_MEMORY_WORDS + \
                                        DEVICE_NAME_MAX_LENGTH + 1)     //0xf04a(1B) 
#define NVM_OFFSET_ESURL_SERVICE_DATA   (NVM_OFFSET_BAT_SERVICE_DATA + 1)//0xf04c(56B)
#define NVM_OFFSET_IBEACON_SERVICE_DATA (NVM_OFFSET_ESURL_SERVICE_DATA +\
                                        sizeof(g_esurl_beacon_data))    //0xf0bc(26B)
#define NVM_OFFSET_BEACON_CONFIG_FLAG   (NVM_OFFSET_IBEACON_SERVICE_DATA + \
                                         sizeof(g_ibeacon_data))        //0xf0f0(1B)
*/
 ...
 *============================================================================*/
app_state EsurlGetAppState()
{
    return g_app_data.state;
}
void BeaconSaveConfigFlagToNvm(uint16 *val,uint16 len)
{
    Nvm_Write(val,len,NVM_OFFSET_BEACON_CONFIG_FLAG);
}
void BeaconGetConfigFlagFromNvm(uint16 *val,uint16 len)
{
    Nvm_Read(val,len,NVM_OFFSET_BEACON_CONFIG_FLAG);
}
/*----------------------------------------------------------------------------*
 *  NAME
 *      appDataInit
...
static void appDataInit(void)
{
    ...
    InitGattData();//g_gatt_data.advert_timer_value=0
    ...
    EsurlBeaconDataInit();//Empty
    
    iBeaconDataInit();//Empty
...
static void readPersistentStore(void)
{
    ...
    if(nvm_sanity == NVM_SANITY_MAGIC)  //not first-time poweron
    {
        ...
        GapReadDataFromNVM(&nvm_offset);//save default  (0xf022~0xf04a/21B)  
        BatteryReadDataFromNVM(&nvm_offset);//&nvm_offset(0xf04c)

        Nvm_Read(&BeaconConfig,1,NVM_OFFSET_BEACON_CONFIG_FLAG);//0xf020
        if((BeaconConfig&0x000f)==0x0001)
        {
            EsurlBeaconReadDataFromNVM(&nvm_offset);//advance the nvm_offset after reading from NVM
            iBeaconReadDataFromNVM(&nvm_offset);
        }
        else
        {
            ...
            iBeaconInitChipReset();
            EsurlBeaconWriteDataToNVM(&nvm_offset);//0xf04e~0xf0c0(58B)
            iBeaconWriteDataToNVM(&nvm_offset);//0xf0c2~0xf0f6(27B)       
        }
    }
    ...
    {   //first time power on
         ...
         * first time.
         */
        //BeaconSaveConfigFlagToNvm(&BeaconConfig,sizeof(BeaconConfig));//save default
        BeaconGetConfigFlagFromNvm(&BeaconConfig,sizeof(BeaconConfig));
        /* Beacon initialisation on chip reset */
        EsurlBeaconInitChipReset();        
        iBeaconInitChipReset();
                
        GapInitWriteDataToNVM(&nvm_offset);//0xf022~0xf04a
        BatteryWriteDataToNVM(&nvm_offset);//0xf04c       
        EsurlBeaconWriteDataToNVM(&nvm_offset);//0xf04e~0xf0c0
        iBeaconWriteDataToNVM(&nvm_offset);//0xf0c2~0xf0f6
    }    
...
static void appStartConnUpdateTimer(void)
{
    if(g_app_data.conn_interval < PREFERRED_MIN_CON_INTERVAL ||
       g_app_data.conn_interval > PREFERRED_MAX_CON_INTERVAL    //500ms/500ms
#if PREFERRED_SLAVE_LATENCY
       || g_app_data.conn_latency < PREFERRED_SLAVE_LATENCY     //4
#endif
      )
    {
...
static void appAdvertTimerHandler(timer_id tid)
{
    ...
    if(g_app_data.app_tid == tid)
    {
        ...
        GattStopAdverts();//GattCancelConnectReq()
...
static void handleSignalGattAddDbCfm(GATT_ADD_DB_CFM_T *p_event_data)
{
    //bool isBeaconConfigFlag=FALSE;
    //TimerDelete(aioReadSolar_tid);aioReadSolar_tid=TIMER_INVALID;

    //Nvm_Read(&BeaconConfig,1,NVM_OFFSET_BEACON_CONFIG_FLAG);   
    /* Handle signal as per current state */
    switch(g_app_data.state)
    {
        case app_state_init:
                                     ...
                                     LM_EV_CONNECTION_COMPLETE_T *p_event_data)
{
    ...
    g_app_data.conn_interval = p_event_data->data.conn_interval;//update connect params
        ...
        switch(g_app_data.state)
        {
            case app_state_fast_advertising:
            {
                ...
                {                   
                    ...
                    Nvm_Read(&BeaconConfig,1,NVM_OFFSET_BEACON_CONFIG_FLAG);
                    if((BeaconConfig&0x000f)==0x0001)//has configed
                    {   //Use old configure
                        if(g_esurl_beacon_data.on_state==0x01)
                        {
                            BeaconConfig&=0x00ff;
                            BeaconConfig|=0x0100;//eddystone
                        }
                        else
                        {
                            BeaconConfig&=0x00ff;
                            BeaconConfig|=0x0200;//ibeacon                            
                        }
                    ...
                    {   //E:1/INT:X/USER:V
                        enableBtnEventMask();
                        SetState(app_state_sleep);//conyinue to sleep  
                    }                       
                }
            }
            break;
                ...
                HCI_EV_DATA_DISCONNECT_COMPLETE_T *p_event_data)
{
    ...
    iBeaconWriteDataToNVM(NULL);//类似掉电前存盘:断连前及时存档发生的修改write
    /* Handle signal as per current state */
    switch(g_app_data.state)
    {
        case app_state_connected:
        ...
        case app_state_disconnecting://连上但没配置暂未处理
        {
            //whitelist_mode wl_mode = whitelist_disabled;
            ...
            if((BeaconConfig&0x000f)==0x0001)
            {
                ...
                BeaconSaveConfigFlagToNvm(&BeaconConfig,1);
                WarmReset();
                //SetState(app_state_beaconing);//beacon_advertising
            }
            else
            {   //E:1/INT:X/USER:V
                enableBtnEventMask();              
...
extern void SetState(app_state new_state)
{
    ...
    app_state old_state = g_app_data.state;//gatt_access.h->app_state
    ...
    if (old_state != new_state)
    {
        ...
        {
            case app_state_init:
                appInitExit();//only add white list after bonding
            break;
            ...
            case app_state_fast_advertising:
                 ...
                 */
                appExitAdvertising();//g_app_data.app_tid
            break;
        ...
        switch (new_state)
        {
            case app_state_fast_advertising:
            {
                ...
                enableWhiteList();
                //LsStartStopAdvertise(FALSE, whitelist_disabled, ls_addr_type_public);
                PioSetDir(PIO_SIDE_CIRCURT,0);//E:1/INT X/USER X
                PioSetEventMask(1L<<PIO_SOLAR_DETECT,pio_event_mode_disable);//prevent enter detect interrupt
            ...
            case app_state_beaconing:
                ...
                /* Start beaconing */
                selfStart_tid=TimerCreate(0*SECOND,TRUE,handleAioReadSolarLevel);
            ...
            case app_state_sleep:
                PioSetDir(PIO_SIDE_CIRCURT,1);//E:0/INT V/USER V
                PioSet(PIO_SIDE_CIRCURT,0);//to capture the solar battery input    
                PioSetEventMask(1L<<PIO_SOLAR_DETECT,pio_event_mode_falling);
                PioSetEventMask(BUTTON_PIO_MASK, pio_event_mode_falling);
                LsStartStopAdvertise(FALSE, whitelist_disabled, ls_addr_type_public);
...
extern void HandlePairingRemoval(void)
{
    ...
    switch(g_app_data.state)
    {
        ...
        case app_state_fast_advertising:
        {
             ...
             */
            GattStopAdverts();//移除bond和白名单后重新快速广播
        }
        break;
...
/*Emitter:pull high,Interrupt(Collector):Disable,User button:Enable*/
void enableBtnEventMask(void)    //beaconing
{   //E:1/INT X/USER V 
...
/*Emitter:pull low,Interrupt(Collector):Enable,User button:Enable*/
void enableBtnIntEventMask(void)    //sleep
{   //E:0/INT V/USER V
    PioSetDir(PIO_SIDE_CIRCURT,1);//output '0'
    PioSet(PIO_SIDE_CIRCURT,0);   
    PioSetEventMask(1L<<PIO_SOLAR_DETECT,pio_event_mode_falling);//prepare for next interrupt
    PioSetEventMask(BUTTON_PIO_MASK, pio_event_mode_falling);   
}
/*Emitter:pull high,Interrupt(Collector):Disable,User button:Disable*/
void disableBtnIntEventMask(void)    //fast_adv
{   //E:1/INT X/USER X
    PioSetDir(PIO_SIDE_CIRCURT,0);
    PioSetEventMask(1L<<PIO_SOLAR_DETECT,pio_event_mode_disable);                   
    PioSetEventMask(BUTTON_PIO_MASK, pio_event_mode_disable);    
...
void gattConnectInit(void)
{
    ...
    //DebugWriteBdAddr(&ra);DebugIfWriteString(" <-random Mac addr\r\n");
#endif 
...
void AppInit(sleep_state last_sleep_state)
{
    ...
    if(1)readPersistentStore();
    
    /*BeaconConfig:[15:8]--branchFlag,
                   [7:4] --savingPowerFlag,
                   [3:0] --isConfigFlag  */  
    if( (BeaconConfig&0xffff)==0xffff )//出厂状态default
    {   //E:1/INT X/USER X
        BeaconConfig&=0x0310;//default:brach=GATT Config(03)/savingPower=ON(1)/isConfig=FALSE(0)
        NvmWrite(&BeaconConfig,1,NVM_OFFSET_BEACON_CONFIG_FLAG);    
    }
    ...
    {        
        ...
        {
            if( (BeaconConfig&0x00f0)==0x0010 ) //省电模式开启
            {
                ...
                {   
                    ...
                    BeaconStart(TRUE);//Eddystone                                          
                    aioReadSolar_tid=TimerCreate(20*SECOND,TRUE,handleAioReadSolarLevel);//循环探测
                }
            }
            else                               //省电模式关闭
            {
        ...
        case 0x0200://ibeacon
        {
            if( (BeaconConfig&0x00f0)==0x0010 )
            {
                uint16 solarLevel=AioRead(AIO0);//AIO2
                if(solarLevel<800)   
                {
                    enableBtnIntEventMask();//E:0/INT V/USER V enable_all_io_int()
                    SetState(app_state_sleep);                        
                }
                else 
                {    
                    enableBtnEventMask();//E:1/INT X/USER V enable_btn_int()
                    g_app_data.state=app_state_beaconing;
        ...
        case 0x0300://GATT config mode
        {   
            disableBtnIntEventMask();//E:1/INT X/USER X disable_all_io_int()
            gattConnectInit();
